...
const PlayerInfo: React.FC<PlayerInfoProps> = ({ player, isCurrentTurn, capturedPieces = [] }) => {
  return (
    <motion.div
      className={cn(
    ...
    >
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-slate-400 text-sm">{player.rating} ELO</p>
        </div>
      </div>
      <div className="mt-2 ml-1 flex flex-wrap gap-x-1">
        {capturedPieces.map((piece, index) => (
            <span key={index} className="captured-piece" style={{ color: piece.color === 'white' ? '#FFF' : '#AAA' }}>
                {pieceMap[piece.type]}
            </span>
        ))}
      </div>
