...
interface SettingsSectionProps {
  ...
  settings: ProfileSettings | AppearanceSettings | AudioSettings | GameSettings;
  onSettingChange: (key: string, value: any) => void;
  fields: Field[];
}

const SettingsSection: React.FC<SettingsSectionProps> = ({ title, icon: Icon, settings, onSettingChange, fields }) => {
  ...
  const renderField = (field: Field) => {
    ...
    switch (field.type) {
      ...
      case 'toggle':
        return (
          <Button
            onClick={() => handleInputChange(field.key, null, field.type)}
      ...
      case 'range':
        return (
          <div className="flex items-center space-x-3">
            ...
            />
            <span className="text-white font-medium w-12 text-center">{value}%</span>
          </div>
        );
